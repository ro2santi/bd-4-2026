...
import { CartManager } from '../cartStore'; // Import Store
...
export default function CartScreen({ navigation }) {
  ...
  // Sinkronkan UI dengan data global setiap kali layar dibuka
  useFocusEffect(
    useCallback(() => {
  ...
  const handleUpdateQty = (id, action) => {
    ...
    setCart([...CartManager.getItems()]); // Refresh tampilan
  };
  ...
  const handleCheckout = async () => {
    if (!buyerName || !buyerPhone) return Alert.alert("Error", "Mohon isi Nama & WA");
    ...
    try {
      ...
      const { error } = await supabase.from('orders').insert([
        { customer_name: buyerName, phone_number: buyerPhone, items: cart, total_price: total, status: 'Baru' }
      ]);
      if (error) throw error;

      // 2. Update CRM Pelanggan
      await supabase.from('customers').upsert([{ name: buyerName, phone: buyerPhone }], { onConflict: 'phone' });
      ...
      <FlatList 
        ...
        renderItem={({ item }) => (
          <View style={styles.cartItem}>
            ...
            <TouchableOpacity onPress={() => handleRemove(item.id)} style={{ marginLeft: 15 }}><MaterialCommunityIcons name="trash-can" size={24} color="#E74C3C" /></TouchableOpacity>
          </View>
        )}
        ListEmptyComponent={<Text style={{textAlign:'center', marginTop: 50, color: '#AAA'}}>Keranjang Kosong. Pilih produk dulu!</Text>}
      />
