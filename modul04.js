            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.detailInfo}>
                ...
                <Text style={styles.detailDesc}>
                  Produk unggulan dari mitra UMKM Hub. Produk ini telah melalui kurasi ketat untuk memastikan kualitas terbaik bagi pelanggan kami.
                </Text>
                <TouchableOpacity style={styles.mainBtn} onPress={() => Alert.alert("Pesanan", "Menghubungkan ke WhatsApp Penjual...")}>
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.navBar}>
          <Text style={styles.brandText}>Dashboard Owner</Text>
          <TouchableOpacity onPress={handleLogout}><Text style={{color: '#E74C3C', fontWeight: 'bold'}}>Keluar</Text></TouchableOpacity>
        </View>
        <ScrollView style={{padding: 20}}>
          <View style={styles.statsRow}>
            <View style={styles.statCard}><Text style={styles.statNum}>{products.length}</Text><Text style={styles.statLabel}>Produk Aktif</Text></View>
            <View style={styles.statCard}><Text style={styles.statNum}>24</Text><Text style={styles.statLabel}>Pesanan Baru</Text></View>
          </View>
          <Text style={styles.sectionTitle}>Ringkasan Bisnis</Text>
          <View style={styles.dummyChart}><MaterialCommunityIcons name="chart-bar" size={50} color="#BDC3C7" /><Text style={{color: '#95A5A6', marginTop: 10}}>Statistik Penjualan (Modul 08)</Text></View>
        </ScrollView>
      </SafeAreaView>
    );
  }
  ...
  if (role === 'owner' && !isLoggedIn) {
    return (
      <View style={styles.landingContainer}>
        ...
        <Text style={styles.landingSubtitle}>Gunakan admin@umkm.com / 123456</Text>
        <View style={styles.inputGroup}>
          <Text style={styles.inputLabel}>Email</Text>
          <TextInput style={styles.formInput} placeholder="Email Bisnis" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />
          <Text style={styles.inputLabel}>Password</Text>
          <TextInput style={styles.formInput} placeholder="Password" value={password} onChangeText={setPassword} secureTextEntry />
        </View>
        <TouchableOpacity style={styles.mainBtn} onPress={handleLogin}><Text style={styles.mainBtnText}>Masuk Ke Dashboard</Text></TouchableOpacity>
  ...
  if (!role) {
    return (
      <View style={styles.landingContainer}>
        ...
        <Text style={styles.landingSubtitle}>Platform Digital UMKM Indonesia</Text>
        <View style={styles.buttonGroup}>
          <TouchableOpacity style={styles.mainBtn} onPress={() => setRole('customer')}><Text style={styles.mainBtnText}>Masuk Pelanggan</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.secondaryBtn, {borderWidth: 2, borderColor: '#3498DB', borderRadius: 16, marginTop: 15}]} onPress={() => setRole('owner')}><Text style={styles.secondaryBtnText}>Pemilik Usaha</Text></TouchableOpacity>
        </View>
      </View>
    );
  }
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      {loading ? <ActivityIndicator size="large" color="#3498DB" style={{flex: 1}} /> : (
        <FlatList
          ...
          ListEmptyComponent={<Text style={{textAlign: 'center', marginTop: 50, color: '#95A5A6'}}>Produk tidak ditemukan...</Text>}
        />
      )}
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  landingTitle: { fontSize: 32, fontWeight: '900', color: '#2C3E50', marginBottom: 10 },
  landingSubtitle: { fontSize: 14, color: '#7F8C8D', textAlign: 'center', marginBottom: 40 },
  buttonGroup: { width: '100%' },
  mainBtn: { backgroundColor: '#3498DB', width: '100%', padding: 18, borderRadius: 16, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', elevation: 3 },
  mainBtnText: { color: '#fff', fontWeight: 'bold', fontSize: 16 },
  secondaryBtn: { padding: 15, alignItems: 'center' },
  secondaryBtnText: { color: '#3498DB', fontWeight: 'bold', fontSize: 16 },
  backLink: { marginTop: 20 },
  backLinkText: { color: '#7F8C8D' },
  navBar: { flexDirection: 'row', justifyContent: 'space-between', padding: 20, alignItems: 'center' },
  brandText: { fontSize: 22, fontWeight: 'bold', color: '#2C3E50' },
  searchContainer: { flexDirection: 'row', backgroundColor: '#F0F3F4', margin: 20, padding: 12, borderRadius: 12, alignItems: 'center' },
  searchInput: { flex: 1, marginLeft: 10, fontSize: 15 },
  catChip: { paddingHorizontal: 15, paddingVertical: 8, backgroundColor: '#F8F9FA', marginRight: 10, borderRadius: 20, borderWidth: 1, borderColor: '#ECF0F1', height: 40 },
  ...
  inputLabel: { fontWeight: 'bold', marginBottom: 5, color: '#34495E', marginLeft: 5 },
  ...
  pCard: { backgroundColor: '#FFF', width: '48%', marginBottom: 15, borderRadius: 15, elevation: 4, overflow: 'hidden' },
  pImg: { width: '100%', height: 120 },
  pContent: { padding: 10 },
  pCat: { fontSize: 9, color: '#3498DB', fontWeight: 'bold', textTransform: 'uppercase' },
  pName: { fontWeight: 'bold', fontSize: 14, marginVertical: 2 },
  ...
  detailName: { fontSize: 26, fontWeight: 'bold', color: '#2C3E50' },
  ...
  statCard: { backgroundColor: '#F0F7FF', width: '48%', padding: 20, borderRadius: 20, alignItems: 'center', borderBottomWidth: 4, borderBottomColor: '#3498DB' },
  statNum: { fontSize: 28, fontWeight: 'bold', color: '#2C3E50' },
